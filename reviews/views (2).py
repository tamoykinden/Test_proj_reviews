...
class CountryViewSet(ExportMixin, viewsets.ModelViewSet):
    ...
    permission_classes = [HasAPIAccessToken]  # Требование: доступ по токену
...
class CommentViewSet(ExportMixin, viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        """
        Настраиваю права доступа в зависимости от действия:
        - create, list, retrieve: публичный доступ
        - update, partial_update, destroy: по токену
        """
        if self.action in ['create', 'list', 'retrieve']:
            permission_classes = []  # Требование: публичный доступ для создания и просмотра
        else:
            permission_classes = [HasAPIAccessToken]  # Требование: токен для изменений
